 ...
 */
export default function computeTotalAmount<T extends AmountDetails | ExpensesWithTotalAmount>(
  record: T
): WithTotalAmount<T> {
  let totalAmount = 0
  ...
  if ("details" in record) {
    ...
    // Vérification de type pour ExpensesWithTotalAmount
    // record est de type ExpensesWithTotalAmount
    ...
    // Si le type n'est ni AmountDetails ni ExpensesWithTotalAmount, une erreur peut être levée ou un comportement par défaut peut être défini
